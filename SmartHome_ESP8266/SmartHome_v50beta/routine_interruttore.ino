...
String Interruttore_ON      = "1on";
String Interruttore_OFF     = "1off";
String Interruttore_ACK_ON  = "1on";
String Interruttore_ACK_OFF = "1off";
String Interruttore_ACK_ONLINE = "online";
...
void setup_Interruttore() {
  ...
  Interruttore[0].topic_ACK = Interruttore1_Topic_ACK;
  Interruttore[1].topic_ACK = Interruttore2_Topic_ACK;
  Interruttore[2].topic_ACK = Interruttore3_Topic_ACK;
  Interruttore[3].topic_ACK = Interruttore4_Topic_ACK;
  Interruttore[4].topic_ACK = Interruttore5_Topic_ACK;
  Interruttore[5].topic_ACK = Interruttore6_Topic_ACK;
  Interruttore[6].topic_ACK = Interruttore7_Topic_ACK;
  Interruttore[7].topic_ACK = Interruttore8_Topic_ACK;
  Interruttore[8].topic_ACK = Interruttore9_Topic_ACK;
  Interruttore[9].topic_ACK = Interruttore10_Topic_ACK;
  ...
  for (int i = 0; i < Interruttore_nodi; i++) {
    ...
    Debug_MSG_LN("RELE    Interruttore " + String(i + 1) + " SU  = GPIO" + String(Interruttore[i].rele));
    Debug_MSG_LN("BOTTONE Interruttore " + String(i + 1) + " SU  = GPIO" + String(Interruttore[i].bottone));
    Debug_MSG_LN("STATO   Interruttore " + String(i + 1) + " SU  = GPIO" + String(Interruttore[i].stato));
    ...
    if (EEPROM_READ_Interruttore_stato_rele(1) == 0) SONOFFDUAL_B_FF();
#endif
...
void ACK_Interruttore() {
  ...
  for (int i = 0; i < Interruttore_nodi; i++) {
    ...
    payload += " alive! " + Interruttore_DEF + " ";
    ...
    else payload = Interruttore_ACK_OFF;
    client.publish((char*) Interruttore[i].topic_ACK.c_str(), (char*) payload.c_str());
    delay(250);
  }
}

void Online_ACK_Interruttore() {
  String payload;
  for (int i = 0; i < Interruttore_nodi; i++) {
    payload = Interruttore_ACK_ONLINE;
    client.publish((char*) Interruttore[i].topic_ACK.c_str(), (char*) payload.c_str());
    delay(250);
  }
}
  ...
  for (int i = 0; i < Interruttore_nodi; i++) {
    if (String(topic) == Interruttore[i].topic) {               // se arriva il comando sul topic "Interruttore1_Topic"
...
#endif
        /*#if defined(ESP01_SERIAL_RELE)
          if (digitalRead(RELE_Interruttore1)) ESP01_SERIAL_RELE_AON();
      ...
      if (message_String == Interruttore_STATO) {                                                     // Topic "Interruttore1_Topic" = "stato"
        ...
        payload += "dB ";
        payload += " RELE=";
        if (digitalRead(Interruttore[i].rele) ^ Flag_inversione_RELE == true ) payload += "ON";
        if (digitalRead(Interruttore[i].rele) ^ Flag_inversione_RELE == false ) payload += "OFF";
        payload += " TIPO=";
        payload += Interruttore[i].tipo_bottone;
        client.publish(ACK_Topic, (char*) payload.c_str());
        delay(100);
      }
    }
