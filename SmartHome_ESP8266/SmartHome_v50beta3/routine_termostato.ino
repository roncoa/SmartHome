...
InfoRecordTermostatoType Termostato[2];

DHT dht1(Termostato1_GPIO_DHT, DHTTYPE, 15);
DHT dht2(Termostato2_GPIO_DHT, DHTTYPE, 15);
...
void setup_Termostato() {
  ...
  Termostato[1].topic = Termostato2_Topic;
  Termostato[1].topic_ACK = Termostato2_Topic;
  Termostato[1].topic_ACK += Termostato_Topic_ACK_suffisso;
  Termostato[1].rele = Termostato2_GPIO_rele;
  Termostato[1].dht = Termostato2_GPIO_DHT;
  Termostato[1].AUTO = false;
  Termostato[1].termostato = 18.00;
...
void sendTemperature() {
  ...
  delay(50);
  if (Termostato_nodi > 1) {
    Termostato[1].h = dht2.readHumidity();
    delay(50);
    Termostato[1].t = dht2.readTemperature(false);                           // Read temperature as Celsius (the default)
    delay(50);
    Termostato[1].f = dht2.readTemperature(true);                            // Read temperature as Fahrenheit (isFahrenheit = true)
    delay(50);
  }
...
void loop_Termostato() {
  ...
  for (int i = 0; i < Termostato_nodi; i++) {
    ...
    if (Termostato_nodi > 1) {
      ...
      display.setFont(ArialMT_Plain_10);
      display.setTextAlignment(TEXT_ALIGN_LEFT);
      display.drawString(30, 17, "Termostato " + String(i + 1));
    }
    display.setFont(ArialMT_Plain_16);
    ...
    display.drawString(0, 45, String(Termostato[i].t) + " Â°C");
    display.setTextAlignment(TEXT_ALIGN_RIGHT);
    display.drawString(128, 27, "UmiditÃ :");
