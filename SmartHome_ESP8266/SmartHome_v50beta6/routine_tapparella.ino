...
TPush Tapparella_TPush_bottone_SU[Tapparella_nodi], Tapparella_TPush_bottone_GIU[Tapparella_nodi], Taparella_timer_refresh_perc;
...
void loop_Tapparella() {
  ...
  for (int i = 0; i < Tapparella_nodi; i++) {
    ...
    if (Taparella_timer_refresh_perc.Wait(2500)) {
...
void Tapparella_cmd_SU (int i, unsigned long target) {
  ...
  delay(Tapparella_TEMPO_INVERSIONE_RELE);                                              // Aspetto
