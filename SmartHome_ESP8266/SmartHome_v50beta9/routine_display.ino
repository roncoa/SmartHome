...
void setup_Display() {
  ...
  for (int i = 0; i < Display_nodi; i++) {
    Debug_MSG_LN("Nodo Display " + String(i + 1) + " : " + String(Display[i].topic));
    if (i < 1) {
...
void ACK_Display() {
  for (int i = 0; i < Display_nodi; i++) {
    ...
    JSONdoc["NODO"] = Display_DEF;
    JSONdoc["Topic"] = Display[i].topic;
    JSONdoc["Topic_ACK"] = Display[i].topic_ACK;
...
void Stato_DIS(int i) {
  DynamicJsonDocument JSONdoc(1024);
  JSONdoc["NODO"] = Display_DEF;
  JSONdoc["RIGA1"] = Display[i].riga_1;
  JSONdoc["RIGA2"] = Display[i].riga_2;
  JSONdoc["RIGA3"] = Display[i].riga_3;
  JSONdoc["RIGA4"] = Display[i].riga_4;
  String payload = "";
  serializeJson(JSONdoc, payload);
