...
#if (Termostato1_tipo_sensore == 1)
DHT dht(Termostato1_GPIO_sensore, DHT22);
#endif
#if (Termostato1_tipo_sensore == 2)
DHT dht(Termostato1_GPIO_sensore, DHT11);
#endif
#if (Termostato1_tipo_sensore == 3)
DS18B20 ds(Termostato1_GPIO_sensore);
#endif
...
void setup_Termostato() {
...
#endif
  Termostato[0].dht = Termostato1_GPIO_sensore;
  ...
  for (int i = 0; i < Termostato_nodi; i++) {
    ...
    Debug_MSG(" sensore termostato " + String(i + 1) + "     = ");
#if (Termostato1_tipo_sensore == 1)
    Debug_MSG("DHT22");
#endif
#if (Termostato1_tipo_sensore == 2)
    ...
    Debug_MSG("DS18B20");
#endif
    Debug_MSG_LN(" GPIO" + String(Termostato[i].dht));
...
#if (Termostato1_tipo_sensore == 1 or Termostato1_tipo_sensore == 2)
...
void ACK_Termostato() {
  for (int i = 0; i < Termostato_nodi; i++) {
    ...
    JSONdoc["NODO"] = Termostato_DEF;
    JSONdoc["Topic"] = Termostato[i].topic;
    JSONdoc["Topic_ACK"] = Termostato[i].topic_ACK;
...
void Stato_TER(int i) {
  DynamicJsonDocument JSONdoc(1024);
  JSONdoc["NODO"] = Termostato_DEF;
  JSONdoc["T"] = Termostato[i].t;
  JSONdoc["H"] = Termostato[i].h;
  JSONdoc["HI"] = Termostato[i].hi;
  if (!Termostato[i].CRONO && Termostato[i].AUTO) JSONdoc["STATO"] = Termostato_ACK_AUTO;
  if (!Termostato[i].CRONO && !Termostato[i].AUTO) JSONdoc["STATO"] = Termostato_ACK_MAN;
  if (Termostato[i].CRONO) JSONdoc["STATO"] = Termostato_ACK_CRONO;
  if (digitalRead(Termostato[i].rele) == true ^ Flag_inversione_RELE) JSONdoc["RELE"] = Termostato_ACK_ON;
  if (digitalRead(Termostato[i].rele) == false ^ Flag_inversione_RELE) JSONdoc["RELE"] = Termostato_ACK_OFF;
  JSONdoc["TERMOSTATO"] = (String)Termostato[i].ta;
  JSONdoc["T1"] = (String)Termostato[i].t1;
  JSONdoc["T2"] = (String)Termostato[i].t2;
  JSONdoc["T3"] = (String)Termostato[i].t3;
  for (int ii = 0; ii <= 23; ii++) {
    JSONdoc["CRONO-" + (String)ii] = (String)Termostato[i].crono[ii];
  }
  String payload = "";
  serializeJson(JSONdoc, payload);
...
void callback_Termostato(char* topic, char* message) {
  ...
  for (int i = 0; i < Termostato_nodi; i++) {
    if (String(topic) ==   Termostato[i].topic) {                                                                             // se arriva il comando sul topic "Termostato1_Topic"
      ...
      for (int ii = 0; ii <= 23; ii++) {
        if (!JSONdoc["CRONO-" + (String)ii].isNull()) {
          Termostato[i].crono[ii] = JSONdoc["CRONO-" + (String)ii];
        }
      }
      if (!JSONdoc["T1"].isNull()) {
        String stringtmp = JSONdoc["T1"];
        Termostato[i].t1 = stringtmp.toFloat();
        Debug_MSG_LN("T1=" + (String)Termostato[i].t1);
      }
      if (!JSONdoc["T2"].isNull()) {
        String stringtmp = JSONdoc["T2"];
        Termostato[i].t2 = stringtmp.toFloat();
        Debug_MSG_LN("T2=" + (String)Termostato[i].t2);
      }
      if (!JSONdoc["T3"].isNull()) {
        String stringtmp = JSONdoc["T3"];
      ...
      if ((String)message == Termostato_TOGGLE) {                     // Topic "Termostato1_Topic" = "toggle"
        // Termostato[i].AUTO = false;
        digitalWrite(Termostato[i].rele, not(digitalRead(Termostato[i].rele)));                                            // Inverto stato RELE
      }
      if (((char)message[0] == 'T' or (char)message[0] == 't') and (char)message[1] == '=' ) {                                  // Topic "Termostato1_Topic" = "T=" o "t="
        ...
        if (Termostato[i].ta<Tmin or Termostato[i].ta>Tmax or isnan(Termostato[i].ta)) Termostato[i].ta = 18;
      ...
      if (((char)message[0] == 'T' or (char)message[0] == 't') and (char)message[1] == '1') {                                  // Topic "Termostato1_Topic" = "T1=" o "t1="
        ...
        Termostato[i].t1 = stringtmp.toFloat();
        if (Termostato[i].t1<Tmin or Termostato[i].t1>Tmax or isnan(Termostato[i].t1)) Termostato[i].t1 = 18;
        Debug_MSG_LN("t1=" + (String)Termostato[i].t1);
      }
      if (((char)message[0] == 'T' or (char)message[0] == 't') and (char)message[1] == '2') {                                  // Topic "Termostato1_Topic" = "T2=" o "t2="
        ...
        stringtmp = stringtmp.substring(3);
        Termostato[i].t2 = stringtmp.toFloat();
        if (Termostato[i].t2<Tmin or Termostato[i].t2>Tmax or isnan(Termostato[i].t2)) Termostato[i].t2 = 18;
        Debug_MSG_LN("t2=" + (String)Termostato[i].t2);
      }
      if (((char)message[0] == 'T' or (char)message[0] == 't') and (char)message[1] == '3') {                                  // Topic "Termostato1_Topic" = "T3=" o "t3="
        ...
        stringtmp = stringtmp.substring(3);
        Termostato[i].t3 = stringtmp.toFloat();
        if (Termostato[i].t3<Tmin or Termostato[i].t3>Tmax or isnan(Termostato[i].t3)) Termostato[i].t3 = 18;
        Debug_MSG_LN("t3=" + (String)Termostato[i].t3);
      }
      /*  DEPRECATED
      ...
      if ((String)message == Termostato_CRONO) {                                // Topic "Termostato1_Topic" = "crono"
...
void Pagina_1(int i) {
  display.setFont(ArialMT_Plain_16);
  display.setTextAlignment(TEXT_ALIGN_CENTER);
  if (!Termostato[i].CRONO && Termostato[i].AUTO) display.drawString(64, 24, "TA = " + String(Termostato[i].ta) + "°C");
  if (!Termostato[i].CRONO && !Termostato[i].AUTO) {
    if (digitalRead(Termostato[i].rele) == 1  ^ Flag_inversione_RELE) display.drawString(64, 24, "Manuale ON");
    else display.drawString(64, 24, "Manuale OFF");
  }
  if (Termostato[i].CRONO && cbtime_set)  {
    now = dstAdjusted.time(nullptr);
    char buffer[80];
  ...
  if (Termostato[i].CRONO && !cbtime_set) display.drawString(64, 24, "Crono -no time-");
#if (Termostato1_tipo_sensore == 1 or Termostato1_tipo_sensore == 2)
  display.setTextAlignment(TEXT_ALIGN_LEFT);
