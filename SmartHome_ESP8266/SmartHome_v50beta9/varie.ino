...
void setup_Status_LED() {
  ...
  digitalWrite(Status_LED_GPIO, 0 ^ Flag_inversione_Status_LED);
#else
...
void Check_PubSubClient() {
  Debug_MSG_LN("MQTT_MAX_PACKET_SIZE: " + (String) MQTT_MAX_PACKET_SIZE);
  Debug_MSG_LN("MQTT_KEEPALIVE: " + (String) MQTT_KEEPALIVE);
  if (MQTT_MAX_PACKET_SIZE < 1024) {
    Debug_MSG_LN("");
    Debug_MSG_LN("*******************************");
    Debug_MSG_LN("* MQTT_MAX_PACKET_SIZE < 1024 *");
    Debug_MSG_LN("*******************************");
    Debug_MSG_LN("");
    unblockingDelay(5000);
  }
...
void loop_Status_LED() {
  ...
  if (TIMER_Status_LED.Wait(250)) {
    ...
    if (lamp == 0) digitalWrite(Status_LED_GPIO, 1 ^ Flag_inversione_Status_LED);
    if (lamp == 1) digitalWrite(Status_LED_GPIO, 0 ^ Flag_inversione_Status_LED);
    if (lamp == 2) digitalWrite(Status_LED_GPIO, 1 ^ Flag_inversione_Status_LED);
    if (lamp == 3) digitalWrite(Status_LED_GPIO, 0 ^ Flag_inversione_Status_LED);
    if (lamp == 4) digitalWrite(Status_LED_GPIO, 0 ^ Flag_inversione_Status_LED);
    if (lamp == 5) digitalWrite(Status_LED_GPIO, 0 ^ Flag_inversione_Status_LED);
    if (lamp == 6) digitalWrite(Status_LED_GPIO, 0 ^ Flag_inversione_Status_LED);
  }
#endif
}
