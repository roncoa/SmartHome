...
//#define Impulso_mantieni_click              // se abilitato(non commentato) mantiene il relè eccitato con tag vicino e/o duante la pressione del bottone
...
void ACK_Impulso() {
  for (int i = 0; i < Impulso_nodi; i++) {
    ...
    JSONdoc["NODO"] = Impulso_DEF;
    JSONdoc["Topic"] = Impulso[i].topic;
    JSONdoc["Topic_ACK"] = Impulso[i].topic_ACK;
...
void HA_discovery_Impulso() {
  ...
  for (int i = 0; i < Impulso_nodi; i++) {
    ...
    HAtopic += "/config";
    DynamicJsonDocument JSONdoc(1024);
    JSONdoc["name"] = Impulso[i].topic;
    JSONdoc["command_topic"] = Impulso[i].topic;
    JSONdoc["payload_on"] = "on";
    JSONdoc["payload_off"] = "off";
    JSONdoc["state_topic"] = Impulso[i].topic_ACK;
    JSONdoc["state_on"] = "on";
...
void Stato_IMP(int i) {
  DynamicJsonDocument JSONdoc(1024);
  JSONdoc["NODO"] = Impulso_DEF;
  if (digitalRead(Impulso[i].rele) ^ (Flag_inversione_RELE == true  )) JSONdoc["STATO"] = Impulso_ACK_ON;
  if (digitalRead(Impulso[i].rele) ^ (Flag_inversione_RELE == false )) JSONdoc["STATO"] = Impulso_ACK_OFF;
  String payload = "";
  serializeJson(JSONdoc, payload);
