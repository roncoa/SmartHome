...
typedef struct {
  ...
  long tempo_max;
  int lock;                                 // lock=1 -> blocca solo comandi wifi , lock=2 -> blocca solo pulsanti ,lock=3 -> blocca tutto
...
void ACK_Tapparella() {
  for (int i = 0; i < Tapparella_nodi; i++) {
    ...
    JSONdoc["NODO"] = Tapparella_DEF;
    JSONdoc["Topic"] = Tapparella[i].topic;
    JSONdoc["Topic_ACK"] = Tapparella[i].topic_ACK;
...
void HA_discovery_Tapparella() {
  ...
  for (int i = 0; i < Tapparella_nodi; i++) {
    ...
    HAtopic += "/config";
    DynamicJsonDocument JSONdoc(1024);
    JSONdoc["name"] = Tapparella[i].topic;
    JSONdoc["command_topic"] = Tapparella[i].topic;
    JSONdoc["payload_open"] = "su";
    JSONdoc["payload_close"] = "giu";
    JSONdoc["payload_stop"] = "stop";
#ifdef Tapparella_percentuale
    JSONdoc["position_topic"] = Tapparella[i].topic_ACK;
    JSONdoc["value_template"] = "{{value_json.PERCENTUALE}}";
    JSONdoc["set_position_topic"] = Tapparella[i].topic;
    JSONdoc["set_position_template"] = "%={{position}}";
#endif
    JSONdoc["availability_topic"] = WillTopicStr;
    String HApayload = "";
...
void Stato_TAP (int i) {
  DynamicJsonDocument JSONdoc(1024);
  JSONdoc["NODO"] = Tapparella_DEF;
  if ((digitalRead(Tapparella[i].rele_SU) ^ (Flag_inversione_RELE == true)) and  (digitalRead(Tapparella[i].rele_GIU) ^ (Flag_inversione_RELE == false ))) JSONdoc["STATO"] = Tapparella_ACK_SU;
  if ((digitalRead(Tapparella[i].rele_SU) ^ (Flag_inversione_RELE == false)) and  (digitalRead(Tapparella[i].rele_GIU) ^ (Flag_inversione_RELE == true ))) JSONdoc["STATO"] = Tapparella_ACK_GIU;
  if ((digitalRead(Tapparella[i].rele_SU) ^ (Flag_inversione_RELE == false)) and  (digitalRead(Tapparella[i].rele_GIU) ^ (Flag_inversione_RELE == false ))) JSONdoc["STATO"] = Tapparella_ACK_STOP;
  if ((digitalRead(Tapparella[i].rele_SU) == true) ^ Flag_inversione_RELE) JSONdoc["RELE_SU"] = "on";
  if ((digitalRead(Tapparella[i].rele_SU) == false) ^ Flag_inversione_RELE) JSONdoc["RELE_SU"] = "off";
  if ((digitalRead(Tapparella[i].rele_GIU) == true) ^ Flag_inversione_RELE) JSONdoc["RELE_GIU"] = "on";
  if ((digitalRead(Tapparella[i].rele_GIU) == false) ^ Flag_inversione_RELE) JSONdoc["RELE_GIU"] = "off";
  if (Tapparella[i].lock == 1) JSONdoc["LOCK"] = "true";
  if (Tapparella[i].lock == 0) JSONdoc["LOCK"] = "false";
  JSONdoc["T_MAX"] = Tapparella[i].tempo_max / 1000;
#ifdef Tapparella_percentuale
  JSONdoc["PERCENTUALE"] = (int)Tapparella[i].percentuale;
#endif
  JSONdoc["LOCK"] = Tapparella[i].lock;
  String payload = "";
  serializeJson(JSONdoc, payload);
  ...
  for (int i = 0; i < Tapparella_nodi; i++) {
    if (String(topic) == Tapparella[i].topic) {
      ...
      if (((char)message[0] == 'L' or (char)message[0] == 'l') and ((char)message[1] == '=')) {                   // Topic "Tapparella_Topic" = "L=" o "l="
        Tapparella[i].lock = atoi(&message[2]);
        EEPROM_WRITE_Tapparella_lock(i);
      }
      if (Tapparella[i].lock == 1 or Tapparella[i].lock == 3) {
        Debug_MSG_LN("Tapparella " + String(i + 1) + " lock " + (String)Tapparella[i].lock);
        return;
      }
...
void loop_Tapparella() {
  ...
  for (int i = 0; i < Tapparella_nodi; i++) {
    ...
    if (Tapparella_TPush_bottone_SU[i].Click(Tapparella_TEMPO_CLICK_ON) and Tapparella[i].bottone_SU != 100) {    // Bottone SU tapparella premuto!
      if (Tapparella[i].lock == 2 or Tapparella[i].lock == 3) {
        Debug_MSG_LN("Tapparella " + String(i + 1) + " lock " + (String)Tapparella[i].lock);
        return;
      }
      Tapparella_cmd_SU(i, Tapparella[i].tempo_max);
...
void EEPROM_READ_Tapparella_lock(int i) {
  ...
  Tapparella[i].lock = eepromRead(EEPROM_Tapparella_lock + i);
  if (Tapparella[i].lock<0 or Tapparella[i].lock>3) {
    Tapparella[i].lock = 0;
    Debug_MSG_LN("Wrong EEPROM lock block!");
    EEPROM_WRITE_Tapparella_lock(i);
  }
  if (Tapparella[i].lock == 0) Debug_MSG_LN("unlock");
  else Debug_MSG_LN("lock " + (String)Tapparella[i].lock);
...
void EEPROM_WRITE_Tapparella_lock(int i) {
  ...
  eepromWrite((EEPROM_Tapparella_lock + i), Tapparella[i].lock);
  EEPROM.commit();
}
